...
it('matches snapshot if on the close position page', () => {
  const wrapper = shallow(<Navigation route={R.CLOSE_POSITION} />);

  expect(wrapper).toMatchSnapshot();
});

it('matches snapshot if on the create position page', () => {
  const wrapper = shallow(<Navigation route={R.CREATE_POSITION} />);

  expect(wrapper).toMatchSnapshot();
});

it('matches snapshot if on the position page', () => {
  const wrapper = shallow(<Navigation route={R.POSITION} />);

  expect(wrapper).toMatchSnapshot();
});

it('matches snapshot if on the profile page', () => {
  const wrapper = shallow(<Navigation route={R.PROFILE} />);
